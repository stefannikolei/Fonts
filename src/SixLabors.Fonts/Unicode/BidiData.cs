...
internal class BidiData
{
    ...
    /// data the runtime reads directly from metadata. The bytes are generated from
    /// the general classification path and a test asserts the two never diverge.
    /// ASCII contains no embedding, override, or isolate initiators, so the fast
    /// path never needs the flag bookkeeping the general path performs.
    /// </summary>
    ...
    /// closing brackets by, zero for non-brackets. The ASCII brackets canonicalize
    /// to themselves, so each pair's key is its closing bracket's value.
    /// </summary>
